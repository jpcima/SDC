...
public:
	...
	void visit(SwitchStatement s) {
		...
		fbody[switchBlock].doSwitch(
			s.location,
			buildExpression(s.expression),
			switchTable,
		);
	}
	...
	void visit(TryStatement s) {
		...
		foreach (c; s.catches) {
			unwindActions ~= UnwindInfo(UnwindKind.Catch, c.statement);
		}
		...
		assert(unwindActions[$ - 1].kind == UnwindKind.Catch);
