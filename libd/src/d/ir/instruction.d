...
public:
	...
	BasicBlockRef newBasicBlock(Name name, BasicBlockRef landingpad) {
		...
		basicBlocks[i] = BasicBlock(name, bbref, landingpad);
		return bbref;
	}
	...
	void dumpBasicBlockName(Context c, const ref BasicBlock b) const {
		import std.stdio;
		write(b.name.toString(c), b.self.index);
	}
	...
	void dump(Context c, const ref BasicBlock b, const ref Instruction i) const {
		...
		final switch(i.op) with(OpCode) {
			...
			case Switch:
				writeln("\tswitch ", i.expr.toString(c));
				write("\t\tdefault: ");
			...
			case Return:
				writeln("\treturn ", i.expr ? i.expr.toString(c) : "void");
				break;
			
			case Throw:
				if (i.expr) {
					writeln("\tthrow ", i.expr.toString(c));
			...
			case Halt:
				writeln("\thalt ", i.expr ? i.expr.toString(c) : "");
				break;
		}
	}
}
...
private:
	...
	this(Name name, BasicBlockRef self, BasicBlockRef landingpad) {
		_name = name;
		_self = self;
	...
	void add(Instruction i) in {
		if (terminate) {
			import std.stdio;
			writeln(cast(void*) &this, '\t', instructions[$ - 1].op);
		}
...
public:
	...
	@property terminate() const {
		return !empty && instructions[$ - 1].terminate;
	}
	...
	void branch(Location location, BasicBlockRef dst) {
		add(Instruction.getTerminator(location, OpCode.Branch));
		successors[0] = dst;
	}
	...
	void branch(
		...
		add(Instruction.getTerminator(location, OpCode.Branch, cond));
		successors[0] = ifTrue;
		successors[1] = ifFalse;
	}
	
	void doSwitch(Location location, Expression e, SwitchTable* switchTable) {
		add(Instruction.getTerminator(location, OpCode.Switch, e));
		this.switchTable = switchTable;
	}
	
	void ret(Location location, Expression e = null) {
		add(Instruction.getTerminator(location, OpCode.Return, e));
	}
	
	void doThrow(Location location, Expression e = null) {
		add(Instruction.getTerminator(location, OpCode.Throw, e));
	}
	
	void doCatch(Location location, CatchTable* catchTable) {
		add(Instruction.getTerminator(location, OpCode.Throw));
		this.catchTable = catchTable;
	}
	
	void halt(Location location, Expression msg = null) {
		add(Instruction.getTerminator(location, OpCode.Halt, msg));
	}
}
...
bool isTerminator(OpCode op) {
	return op >= OpCode.Branch;
}

unittest {
	assert(!isTerminator(OpCode.Alloca));
	assert(!isTerminator(OpCode.Evaluate));
	
	assert(isTerminator(OpCode.Branch));
	assert(isTerminator(OpCode.Return));
	assert(isTerminator(OpCode.Throw));
	assert(isTerminator(OpCode.Switch));
}
...
private:
	...
	static getTerminator(Location location, OpCode op) in {
		assert(isTerminator(op), "Expected terminator");
	} body {
		Instruction i;
		i.location = location;
		i.op = op;
	...
	static getTerminator(Location location, OpCode op, Expression e) in {
		assert(isTerminator(op), "Expected terminator");
	} body {
		Instruction i;
		i.location = location;
