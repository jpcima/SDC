...
struct StatementGen {
	...
	void visit(BasicBlockRef b) {
		...
		foreach(i; range(fbody, b)) {
			...
			case Switch:
				auto switchTable = fbody[b].switchTable;
				auto e = genExpression(i.expr);
	...
	private LLVMBasicBlockRef genLandingPad(BasicBlockRef srcBlock) {
		...
		bool cleanup = instrs[0].op != OpCode.Throw;
		
		LLVMValueRef[] clauses;
		if (instrs[$ - 1].op == OpCode.Throw && !instrs[$ - 1].expr) {
